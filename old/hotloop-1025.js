...
function acquireInputs(p5, frameData, state, uiControls, PANEL_SIZE) {
    ...
    if (procW > 0 && (procW !== state.lastProcW || procH !== state.lastProcH)) {
        // FIX: Use Object.keys().forEach() for robust iteration over canvases.
        Object.keys(state.canvases).forEach(key => {
...
function computeReferenceCoeffs(p5, frameData, state, uiControls, workers) { // FIX: Added p5 parameter
    const startTime = performance.now();
...
function updateVisualizations(p5, frameData, state, uiControls) { // FIX: Added p5 parameter
    const startTime = performance.now();
...
async function runComputationLoop(p5, state, uiControls, workers) {
    const { PANEL_SIZE, NUM_WORKERS } = state.constants;
    while (true) {
        ...
        try {
            frameData = acquireInputs(p5, frameData, state, uiControls, PANEL_SIZE);
            frameData = computeReferenceCoeffs(p5, frameData, state, uiControls, workers); // FIX: Pass p5
            ...
            updateVisualizations(p5, frameData, state, uiControls); // FIX: Pass p5
        } finally {
