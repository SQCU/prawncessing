...
function acquireInputs(p5, frameData, state, uiControls) {
    ...
    // FIX: Re-create canvases if size changes, instead of resizing. This is more robust.
    if (procW > 0 && (procW !== state.lastProcW || procH !== state.lastProcH)) {
        Object.values(state.canvases).forEach(c => c && c.remove()); // Clean slate
        ...
        state.canvases = {
            ...
            viz: p5.createGraphics(PANEL_SIZE, PANEL_SIZE).parent('panel-viz')
        };
