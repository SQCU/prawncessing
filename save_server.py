...
class SaveServerHandler(http.server.SimpleHTTPRequestHandler):
    def do_POST(self):
...
with socketserver.TCPServer(("", PORT), SaveServerHandler) as httpd:
    print(f"Save server started at localhost:{PORT}")
    httpd.serve_forever()