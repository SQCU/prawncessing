...
class DownscaleService(WorkerService):
    def __init__(self, name="DownscaleService", input_type="image/jpeg", output_type="image/jpeg"):
        super().__init__(name, "downscaler", input_type, output_type)

    def process_job(self, job_data):
        try:
            # Intentionally raise an error for demonstration
            raise ValueError("Simulated Downscale Error")
            print(f"[{self.name}] Downscaling input: {job_data.get('input_data', 'No input')}")
            time.sleep(0.1) # Simulate work
            return b"downscaled_image_data"
